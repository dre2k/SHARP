...
We will apply some of the methods discussed in lecture on the **HELIX** data set. 

Goals:

* Briefly discuss simple interaction test examples
* Introduce `BinaryDosage`/`GxEScanR`
* Conduct a 'genome-wide' scan yourselves


# HELIX study
<!-- ### Data descriptions for interactions -->

As previously mentioned, the HELIX study is a collaborative project across six longitudinal population-based birth cohort studies in six European countries (France, Greece, Lithuania, Norway, Spain, and the United Kingdom).  

In this example, we will use data from the subcohort of 1,301 mother-child pairs (**N = 1,122 with complete data**), specifically exposure and genomic data from children 6-11 years old, to investigate GxE interactions between fetal alcohol/smoking exposures and genetic variants, and their influence on future childhood BMI.  
...
Throughout this workshop, we'll be using `HELIX.MultiAssayExperiment`, an R object that stores multiple data sets measured on the same individuals (e.g. epidemiological data, a number of *omic* experiments, etc)

We will begin by obtaining the variables we are interested in, and joining them into a `data.frame` for convenience  
...
We can also fit a case-only model, which is shown to be more powerful provided the G-E independence assumption is met. The polytomous regression model is in the form $$logit(Pr G=g|E, D=1) = \gamma_0 + \gamma_1 + \gamma_EE$$ although in this example I will use a linear regression model 
...
Perform GxE analysis using `GxEScanR` to assess interactions between maternal smoking (dichotomous variables no/yes) + genetic variants in determining childhood BMI.  
...
* Is using genetic main effect ranks for step 1 'filtering' sufficient for identify potential GxE interactions?
