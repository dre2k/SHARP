---
title: 'SHARP Multiomics Workshop: Exposome Challenge Data: Polygenic Risk Scores'
author: "David Conti"
date: "`r Sys.time()`"
output:
  html_document:
...
M <- 10 # number of PRS snps for main effects
PRS.SNP.names <- paste("SNP", read.table("../CausalSNPs.txt", sep=" ", header=F), sep=".")[1:M]
...
if(PRS.univariate) {
  PRS.uni.results <- t(sapply(1:ncol(X), FUN=function(p) {  # using index p faciliate write
    g <- X[,p]
    ...
    write.table(t(c(paste("SNP", p, sep="."), c.reg)), file="PRS.UnivariateResultsGenome.txt", append=ifelse(p==1, F, T), quote=F, sep="\t", col.names=ifelse(p==1, T, F), row.names=F)
    return(c.reg)                         # to avoid potential memory issues only return coefficients if small number of exposures
  }, simplify=T))
  PRS.uni.results <- data.frame(paste("SNP", 1:ncol(X), sep="."),PRS.uni.results)
...
if(PRS.univariate) { kable(PRS.uni.results[PRS.uni.results$P.value<0.05,], align="c", row.names=FALSE, col.names=c("E:SNP","Estimate", "SD","Z statistic", "P value"))}
```
