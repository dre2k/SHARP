...
We investiagate the GxE interactions for 
...
if(GxE.univariate) {
  int.results <- t(sapply(1:ncol(X), FUN=function(p) {  # using index p faciliate write
    ...
    c.reg <- s.reg$coef["g:E",]             # select the coefficients
  ...
  int.results <- data.frame(paste("E:SNP", 1:ncol(G), sep="."),int.results)
  names(int.results)[1] <- "SNP.Names"
}
...
if(GxE.univariate) { kable(int.results[int.results$Pr...z..<0.05,], digits=3, align="c", row.names=FALSE, col.names=c("E:SNP","Estimate", "SD","Lower 95% CI", "Upper 95% CI"))}
```

##### GxE Univariate Manhattan Plot:
```{r GxE: Univariate MH plot}
neglog.pvalues <- -log10(int.results$Pr...z..)
