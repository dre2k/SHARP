...
* Conduct scan of maternal smoking and childhood BMI (applied)
...
* **e3_alcpreg_yn_None (Exposure)** - Maternal alcohol use during pregnancy
  + 0 = Never
  + 1 = Ever
* **e3_asmokcigd_p_None (Exposure)** - Maternal smoking during pregnancy (dichotomized)
  + 0 = Never
  + 1 = Ever
...
Case-only models can be more powerful than traditional logistic regression models, provided G-E independence assumption is met.  

One can use polytomous regression model is in the form $$logit(Pr G=g|E, D=1) = \gamma_0 + \gamma_1 + \gamma_EE$$

or a linear model in the form $$G = \gamma_0 + \gamma_EE$$

```{r}
caseonly <- which(dataset$hs_bmi_c_cat == "Overweight/Obese")

SNP.1 <- as.numeric(X[,1][caseonly])
...
* `BinaryDosage` file 
* epidemiological data text file - first two columns must contain ID and outcome variables, the last column must contain the interaction variable. Indicator variables must be created manually  
...
# ---- some checks ---- #
table(bin) # bin sizes
alphaBin # bin step 2 sig threshold
...
![](images/descriptive_table_smoke.png)
<br>

## Univariate analysis

```{r, echo = F}
x <- readRDS("data/univariate.results.smoke.rds")
kable(x) %>% 
  kable_styling()
```
...
### Case only
```{r, echo = T}
plot_qq(output, 'lrtcase', 1, print_yloc = 3)
```
